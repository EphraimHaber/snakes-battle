...
class GameGraphics:
    def __init__(self, ai_classes_available) -> None:
        ...
        self.borders = {
            ...
            'right': [
                ...
                (
                    (self.board_size[0] - settings.BORDER_THICKNESS) * self.cell_size - 1,
    ...
    def _get_cell_coordinates(self, cell_pos):
        top_left = (cell_pos[0] * self.cell_size - 1, cell_pos[1] * self.cell_size - 1)
        top_right = (cell_pos[0] * self.cell_size + self.cell_size - 1, cell_pos[1] * self.cell_size - 1)
        bottom_left = (cell_pos[0] * self.cell_size - 1, cell_pos[1] * self.cell_size + self.cell_size - 1)
        bottom_right = (
            cell_pos[0] * self.cell_size + self.cell_size - 1,
    ...
    def _draw_snake(self, snake, margin=4):
        ...
        for i in range(len(snake._body_position)):
            ...
            # Drawing the rest of the body.
                ...
                if (
                    i < len(snake._body_position) - 1
                    and snake._body_position[i - 1][1] != snake._body_position[i + 1][1]
                    and snake._body_position[i - 1][0] != snake._body_position[i + 1][0]
                ):
    ...
    def _draw_background_lines(self):
        for column in range(self.board_size[0] - settings.BORDER_THICKNESS * 2 + 1):
            start_pos = (
                (column + settings.BORDER_THICKNESS) * self.cell_size - 1,
    ...
    def _draw_scoreboard(self, board):
        ...
        for i, snake in enumerate(all_snakes):
            ...
            score_position = (
                self.SCOREBOARD_STARTING_POSITION[0],
                self.SCOREBOARD_STARTING_POSITION[1]
    ...
    def draw_menu(self, ai_classes_available, events):
        ...
        buttons_start_position = (
            screen_size[0] * (1 / 10 + settings.MENU_BUTTONS_SHIFT_LEFT),
            settings.CLASS_NAME_START_Y,
            300,
            100,
        )
        ...
        for index, class_dict in enumerate(ai_classes_available):
            ...
            self.surface.blit(
                text_surface,
                (settings.CLASS_NAME_START_X, settings.CLASS_NAME_START_Y + settings.CLASS_NAME_SPACING * (index + 1)),
            )
...
class Button:
    def __init__(
        ...
        font,
        surface,
        width=settings.BUTTONS_WIDTH,
        height=settings.BUTTONS_HEIGHT,
        color=settings.MENUS_BUTTON_COLOR,
    ):
        self.x = x
        self.y = y
        self.text = text
